...
/* 正点原子LCD屏硬件ID */
enum ATK_LCD_TYPE {
	ATK4342 = 0, // （000） 4.3寸480*272
	ATK4384 = 4, // （100） 4.3寸800*480
	ATK7084 = 1, // （001） 7寸800*480
	ATK7016 = 2, // （010） 7寸1024*600
	ATK1018 = 5, // （101） 10寸1280*800
};

#define connector2dpi(c) container_of(c, struct atk_dpi, connector)
#define encoder2dpi(e) container_of(e, struct atk_dpi, encoder)

struct atk_dpi {
	struct device *dev; // 设备结构体指针
	struct drm_encoder encoder; // 显示编码器
	struct drm_connector connector; // 显示连接器
	struct videomode *vm; // 视频模式
	struct clk *pclk; // 时钟控制器
};

/* 私有函数声明 */
static enum drm_connector_status atk_dpi_detect(struct drm_connector *connector,
						bool force);
...
static int lcd_get_modes(struct drm_connector *connector)
{
	struct atk_dpi *dpi = connector2dpi(connector);
...
static void atk_dpi_mode_set(struct drm_encoder *encoder,
			     ...
			     struct drm_display_mode *adjusted_mode)
{
	struct atk_dpi *dpi = encoder2dpi(encoder);
...
static int atk_dpi_bind(struct device *dev, struct device *master, void *data)
{
	...
	for_each_child_of_node (dev->of_node, port) {
		if (!port->name || of_node_cmp(port->name, "ports")) {
			DRM_DEBUG_DRIVER(
				"port name is null or node name is not ports!\n");
			continue;
		}
		encoder->possible_crtcs |=
			drm_of_find_possible_crtcs(drm_dev, port);
	}
...
static int atkencoder_init_dt(struct atk_dpi *dpi)
{
	...
	for (i = 0; i < 3; i++) {
		...
		lcd_id |= (gpio_get_value_cansleep(gpios[i])
			   << i); // 读取 GPIO 数值
	}
