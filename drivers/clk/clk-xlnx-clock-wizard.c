...
static const unsigned long clk_wzrd_max_freq[] = { 800000000UL, 933000000UL,
						   1066000000UL };
				 ...
				 unsigned long parent_rate)
{
	...
	for (m = WZRD_M_MIN; m <= WZRD_M_MAX; m++) {
		for (d = WZRD_D_MIN; d <= WZRD_D_MAX; d++) {
			...
			if (vco_freq >= WZRD_VCO_MIN &&
			    vco_freq <= WZRD_VCO_MAX) {
				for (o = WZRD_O_MIN; o <= WZRD_O_MAX; o++) {
					freq = DIV_ROUND_CLOSEST_ULL(vco_freq,
								     o);
					diff = abs(freq - rate);
				       ...
				       unsigned long parent_rate)
{
	...
	vco_freq = DIV_ROUND_CLOSEST((parent_rate * divider->valuem),
				     divider->valued);
					      ...
					      unsigned long parent_rate)
{
	...
	div = DIV_ROUND_CLOSEST(d * (WZRD_FRAC_POINTS * o + f),
				WZRD_FRAC_POINTS);
...
static struct clk *
clk_wzrd_register_divf(struct device *dev, const char *name,
		       const char *parent_name, unsigned long flags,
		       void __iomem *base, u16 offset, u8 shift, u8 width,
		       u8 clk_divider_flags, u32 div_type, spinlock_t *lock)
{
...
static struct clk *
clk_wzrd_register_divider(struct device *dev, const char *name,
			  const char *parent_name, unsigned long flags,
			  void __iomem *base, u16 offset, u8 shift, u8 width,
			  u8 clk_divider_flags, u32 div_type, spinlock_t *lock)
{
...
static int clk_wzrd_probe(struct platform_device *pdev)
{
	...
	if (nr_outputs == 1) {
		clk_wzrd->clkout[0] = clk_wzrd_register_divider(
			...
			__clk_get_name(clk_wzrd->clk_in1), 0, clk_wzrd->base,
			WZRD_CLK_CFG_REG(3), WZRD_CLKOUT_DIVIDE_SHIFT,
			WZRD_CLKOUT_DIVIDE_WIDTH,
			CLK_DIVIDER_ONE_BASED | CLK_DIVIDER_ALLOW_ZERO, DIV_ALL,
			&clkwzrd_lock);
	...
	clk_wzrd->clks_internal[wzrd_clk_mul_div] = clk_register_divider(
		&pdev->dev, clk_name,
		__clk_get_name(clk_wzrd->clks_internal[wzrd_clk_mul]), flags,
		ctrl_reg, 0, 8, CLK_DIVIDER_ONE_BASED | CLK_DIVIDER_ALLOW_ZERO,
		&clkwzrd_lock);
	...
	for (i = nr_outputs - 1; i >= 0; i--) {
		...
		if (!i)
			clk_wzrd->clkout[i] = clk_wzrd_register_divf(
				&pdev->dev, clkout_name, clk_name, flags,
			...
			clk_wzrd->clkout[i] = clk_wzrd_register_divider(
				&pdev->dev, clkout_name, clk_name, 0,
				clk_wzrd->base, (WZRD_CLK_CFG_REG(2) + i * 12),
				WZRD_CLKOUT_DIVIDE_SHIFT,
				WZRD_CLKOUT_DIVIDE_WIDTH,
				CLK_DIVIDER_ONE_BASED | CLK_DIVIDER_ALLOW_ZERO,
...
out:
	...
	if (clk_wzrd->speed_grade) {
		...
		ret = clk_notifier_register(clk_wzrd->clk_in1, &clk_wzrd->nb);
		if (ret)
			dev_warn(&pdev->dev,
