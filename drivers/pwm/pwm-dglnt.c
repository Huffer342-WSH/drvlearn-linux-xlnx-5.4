 ...
 * @return					: 无
 */
static inline void dglnt_pwm_writel(struct dglnt_pwm_dev *dglnt_pwm, u32 reg, u32 val)
{
 ...
 * @return					: 成功返回0，失败返回负数
 */
static int dglnt_pwm_config(struct pwm_chip *chip, struct pwm_device *pwm, int duty_ns, int period_ns)
{
    int duty, period;
...
static int dglnt_pwm_remove(struct platform_device *pdev)
{
    ...
    /* 禁止PWM输出 */
    for (i = 0; i < dglnt_pwm->chip.npwm; i++) dglnt_pwm_writel(dglnt_pwm, PWM_AXI_CTRL_REG_OFFSET, 0);
