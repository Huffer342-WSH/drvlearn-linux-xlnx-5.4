 ...
 * @return					: 无
 */
static inline void dglnt_pwm_writel(struct dglnt_pwm_dev *dglnt_pwm, u32 reg,
				    u32 val)
{
...
static int dglnt_pwm_probe(struct platform_device *pdev)
{
	...
	dglnt_pwm_writel(dglnt_pwm, PWM_AXI_CTRL_REG_OFFSET,
			 0); // 先禁止PWM输出
