...
static int ft5426_write(struct ft5426_dev *ft5426, u8 addr, u8 *buf, u16 len)
{
...
static int ft5426_read(struct ft5426_dev *ft5426, u8 addr, u8 *buf, u16 len)
{
	struct i2c_client *client = ft5426->client;
...
static int ft5426_reset(struct ft5426_dev *ft5426)
{
	struct i2c_client *client = ft5426->client;
	u8 data;
	int ret;
	...
	ret = devm_gpio_request_one(&client->dev, ft5426->reset_gpio,
				    GPIOF_OUT_INIT_LOW, "ft5426 reset");
	if (ret < 0)
		return ret;
	...
	data = 0;
	ret = ft5426_write(ft5426, FT5426_DEVIDE_MODE_REG, &data, 1);
	if (ret)
		return ret;

	data = 1;
	ret = ft5426_write(ft5426, FT5426_ID_G_MODE_REG, &data, 1);
	if (ret)
		return ret;
...
{
	struct ft5426_dev *ft5426 = dev_id;
	u8 rdbuf[30] = { 0 };
	int i, type, x, y, id;
	bool down;
	int ret;
...
static int ft5426_register_irq(struct ft5426_dev *ft5426)
{
	struct i2c_client *client = ft5426->client;
	int ret;
	...
	/* 从设备树中获取中断管脚 */
	ft5426->irq_gpio =
		of_get_named_gpio(client->dev.of_node, "interrupt-gpio", 0);
	...
	/* 申请使用管脚 */
	ret = devm_gpio_request_one(&client->dev, ft5426->irq_gpio, GPIOF_IN,
				    "ft5426 interrupt");
	if (ret < 0)
		return ret;

	/* 注册中断服务函数，在线程中执行ft5426_isr() */
			...
			const struct i2c_device_id *id)
{
	...
	/* 实例化一个struct ft5426_dev对象 */
	ft5426 = devm_kzalloc(&client->dev, sizeof(struct ft5426_dev),
			      GFP_KERNEL);
	if (!ft5426) {
		dev_err(&client->dev,
			"Failed to allocate ft5426 driver data.\n");
		return -ENOMEM;
	}
	...
	/* 申请、注册中断服务函数 */
	ret = ft5426_register_irq(ft5426);
	if (ret)
		return ret;
...
static int ft5426_remove(struct i2c_client *client)
{
	struct ft5426_dev *ft5426 = i2c_get_clientdata(client);
...
MODULE_AUTHOR("Huffer <718007138@qq.com>");
